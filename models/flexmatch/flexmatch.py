...
from .mce import MatrixCrossEntropyVectorLogFYFYFYFX, MatrixCrossEntropyVectorLogFYFYFXFX, MatrixCrossEntropyMatrixLogFYFYFXFX, MatrixCrossEntropyNoCenterFYFYFXFX, MatrixCrossEntropyNoCenterFYFYFYFX, MatrixCrossEntropyMMFYFYFXFX
...
class FlexMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u, \
                 hard_label=True, t_fn=None, p_fn=None, it=0, num_eval_iter=1000, tb_log=None, logger=None, args=None):
        """
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                logits, features = self.model(inputs, ood_test=True)
                logits_x_lb = logits[:num_lb]
                logits_x_ulb_w, logits_x_ulb_s = logits[num_lb:].chunk(2)
                features_ulb_w, features_ulb_s = features[num_lb:].chunk(2)
                sup_loss = ce_loss(logits_x_lb, y_lb, reduction='mean')
                print("Shape of logits_x_lb:", logits_x_lb.shape) #[64, 10]
                print("Shape of y_lb:", y_lb.shape) # [64]
                print("y_lb:", y_lb)
                # get a [64, 10] matrix for one-hot y_lb
                # y_lb_2d = F.one_hot(y_lb, num_classes= logits_x_lb.shape[1]).float()
                # sup_mce_loss = self.mce_loss(y_lb, logits_x_lb, use_feature=False)
                sup_mce_loss = 0
                ...
                unsup_loss, mask, select, pseudo_lb, p_model, pseudo_label = consistency_loss(logits_x_ulb_s,
                                                                                logits_x_ulb_w,
                                                                                classwise_acc,
                                                                                p_target,
                                                                                p_model,
                ...
                indices = torch.argwhere(select).reshape(-1)
                # print(indices)
                logits_x_ulb_s_masked = torch.index_select(logits_x_ulb_s, 0, indices)
                pseudo_lb_masked = torch.index_select(pseudo_lb, 0, indices)
                # print("Shape of logits_x_ulb_s_masked:", logits_x_ulb_s_masked.shape)
                # print("Shape of pseudo_lb_masked:", pseudo_lb_masked.shape)
                # print("pseudo_lb_masked:", pseudo_lb_masked)
                features_ulb_w_masked = torch.index_select(features_ulb_w, 0, indices)
                features_ulb_s_masked = torch.index_select(features_ulb_s, 0, indices)
                # print(features_ulb_w_masked.shape)
                print(pseudo_lb_masked)

                if self.use_feature:
                    unsup_mce_loss = self.mce_loss(features_ulb_w_masked, features_ulb_s_masked, use_feature=True)
                else:
                    unsup_mce_loss = self.mce_loss(pseudo_lb_masked, logits_x_ulb_s_masked, use_feature=False)
                # print(unsup_mce_loss)

                if x_ulb_idx[select == 1].nelement() != 0:
                    selected_label[x_ulb_idx[select == 1]] = pseudo_lb[select == 1]

                total_loss = sup_loss + sup_mce_loss + self.lambda_u * (unsup_loss + self.mce_gamma * unsup_mce_loss)
            ...
            tb_dict['train/unsup_mce_loss'] = unsup_mce_loss.detach()
